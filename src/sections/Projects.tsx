...
export default function Projects() {
  return (
    <div className="w-full h-full flex flex-col justify-center max-w-4xl mx-auto px-4 md:px-0 py-4 select-none relative">
      ...
      {/* Timeline Container (Prominent thick brown border and spacing) */}
      <div className="relative border-l-6 border-[#6F5E53] pl-6 md:pl-8 ml-3 md:ml-4 space-y-10">
        {projectsList.map((project, idx) => (
          <div key={idx} className="relative group">
            {/* Timeline Circle */}
            <div className="absolute -left-9.5 md:-left-11.5 top-2 w-4 h-4 rounded-full bg-[#FAF6F0] border-3 border-[#6F5E53] group-hover:bg-[#E6B800] group-hover:border-[#1E1B18] transition-all duration-300"></div>

            {/* Project Box */}
            <div className="bg-white/50 border-2 border-[#E6DFD5] p-6 md:p-8 rounded-2xl shadow-lg group-hover:shadow-2xl group-hover:border-[#6F5E53] transition-all duration-300">
              <div className="flex flex-col sm:flex-row gap-6 items-start">

                {/* Project Info (Left side) */}
                <div className="flex-1">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    <h3 className="text-xl md:text-2xl font-bold text-[#1E1B18] group-hover:text-[#6F5E53] transition-colors duration-300">
                      {project.title}
                    </h3>
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-1.5 text-xs font-semibold rounded-full bg-[#FAF6F0] border border-[#E6DFD5] text-[#4F4A45] hover:bg-[#1E1B18] hover:text-[#FAF6F0] hover:border-[#1E1B18] transition-all duration-300 shrink-0"
                    >
                  ...
                  <p className="text-sm md:text-base text-[#4F4A45] mb-5 leading-relaxed font-sans">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {project.tech.map((t) => (
                      <span
                        key={t}
                        className="px-3 py-1 text-xs font-mono bg-[#E6DFD5]/60 text-[#6F5E53] rounded-md border border-[#E6DFD5]"
                      >
                        {t}
                      </span>
                    ))}
                ...
                {/* Project Image Container (Slightly larger thumbnail on the right) */}
